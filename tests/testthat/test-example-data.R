test_that("the shipped example data generates a valid Winsteps input set", {
  ...
  expect_equal(nrow(responses), 360)
  expect_equal(nrow(anchor_tbl), 12)
  expect_equal(length(unique(responses$person_id)), 30)
  expect_equal(sum(is.na(responses$score)), 5)
  ...
  expect_length(result$contents$data, 30)
  ...
  # the five missing responses reach the file as the missing code
